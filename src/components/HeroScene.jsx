...
function GlassKnot({ reducedMotion }) {
  ...
  const uniforms = useMemo(
    () => ({
      uTime: { value: 0 },
      uDistort: { value: 0.18 },
      uColorA: { value: new THREE.Color('#F54F1B') },
      uColorB: { value: new THREE.Color('#F54F1B') },
      uOpacity: { value: 0.9 },
    }),
    []
  );
...
function SceneLights() {
  return (
    <>
      ...
      <pointLight position={[4, 3, 5]} intensity={12} color="#F54F1B" />
      <pointLight position={[-4, -2, -3]} intensity={8} color="#F54F1B" />
    </>
  );
}
